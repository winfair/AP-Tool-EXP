...
(() => {
  ...
  function needObserverRefetch(lat, lon){
    const Ls = AP.observerSources;
    if(Ls.lastLat == null || Ls.lastLon == null) return true;
    const dLat = Math.abs(lat - Ls.lastLat);
    const dLon = Math.abs(lon - Ls.lastLon);
  ...
  function update(){
    ...
    if(altPrev){
      altPrev.textContent = `Alt mode: ${AP.settings.altMode.toUpperCase()} · Obs=${obsElev!=null ? obsElev.toFixed(1)+" m" : "–"} (${obsSrc}) · Hinst=${(AP.settings.instrumentHeight||0).toFixed(1)} m`;
    }
  ...
  function setup(retry=0){
    ...
    btnMap.addEventListener("click", ()=>{
      ...
      if(!map){
        ...
        setMapStatus("Tap on the map to set target.");
  ...
  // Kick everything off once scripts & DOM are ready (defer + small delay)
